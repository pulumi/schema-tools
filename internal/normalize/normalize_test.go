...
func TestNormalizeDetectsInCodegenAliasMigration(t *testing.T) {
	t.Parallel()

	oldToken := "pkg:index:BucketAclV2"
	newToken := "pkg:index:BucketAcl"
	...
	newMetadata := mustParseMetadataJSON(t, `{
		"auto-aliasing": {
			"resources": {
				"pkg_bucket_acl": {
					"current": "pkg:index:BucketAcl",
					"majorVersion": 7,
					"past": [
						{
							"name": "pkg:index:BucketAclV2",
							"inCodegen": true,
							"majorVersion": 7
						}
					]
...
func TestNormalizeRenamedLeafUpdatesRequiredInputs(t *testing.T) {
	t.Parallel()
	...
	oldSchema := schema.PackageSpec{
		Resources: map[string]schema.ResourceSpec{
			token: {
				InputProperties: map[string]schema.PropertySpec{
					"ipAddress": {TypeSpec: schema.TypeSpec{Type: "string"}},
				},
				RequiredInputs: []string{"ipAddress"},
			},
		},
	}
	newSchema := schema.PackageSpec{
		Resources: map[string]schema.ResourceSpec{
			token: {
				...
				RequiredInputs: []string{"ipAddresses"},
			},
		},
	}
	...
	oldMetadata := mustParseMetadataJSON(t, `{
		"auto-aliasing": {
			"resources": {
				"pkg_widget": {
					"current": "pkg:index:Widget",
					"fields": {"ip_addresses": {"maxItemsOne": true}}
				}
			},
			"datasources": {}
		}
	...
	newMetadata := mustParseMetadataJSON(t, `{
		"auto-aliasing": {
			"resources": {
				"pkg_widget": {
					"current": "pkg:index:Widget",
					"fields": {"ip_addresses": {"maxItemsOne": false}}
				}
			},
			"datasources": {}
		}
	...
	normalizedResource := normalized.NewSchema.Resources[token]
	require.Contains(t, normalizedResource.InputProperties, "ipAddress")
	require.NotContains(t, normalizedResource.InputProperties, "ipAddresses")
	require.Equal(t, []string{"ipAddress"}, normalizedResource.RequiredInputs)
}
